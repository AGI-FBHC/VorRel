...
from model.arfem import ARFEM
from model.vorrel_net import WeightedBinaryCrossEntropyLoss
...
class VorRelTrainer(nn.Module):
    def __init__(self, config):
        ...
        self.use_focal = config.get('model', {}).get('use_focal_loss', False)
        self.focal_gamma = config.get('model', {}).get('focal_gamma', 2.0)

        self.embedding_layer = FeatureEmbedding(hidden_dim, dropout=0.1)

        self.arfem = ARFEM(config)

        self.pos_weight = config.get('model', {}).get('pos_weight', 3.0)
        self.label_smoothing = config.get('model', {}).get('label_smoothing', 0.0)
        self.edge_types = config.get('model', {}).get('edge_types',
            ['voronoi', 'hydrogen_bond', 'hydrophobic', 'salt_bridge'])

        if self.use_focal:
            self.focal_loss = FocalLoss(pos_weight=self.pos_weight, gamma=self.focal_gamma)
        else:
    ...
    def forward(self, batch, return_edge_attn=False):
        ...
        edge_map = {
            'voronoi': voronoi,
            'hydrogen_bond': hb if hb is not None else torch.zeros_like(voronoi),
            'hydrophobic': hp,
            'salt_bridge': sb if sb is not None else torch.zeros_like(voronoi),
        }
        raw_edges = {
            et: edge_map[et].to(device)
        ...
        predictions, logits = outputs[:2]
        return predictions, logits

    def compute_loss(self, logits, targets):
        """Compute loss with optional Focal Loss and Label Smoothing."""
...
def train_epoch(model, dataloader, optimizer, device):
    ...
    pbar = tqdm(dataloader, desc="训练", mininterval=5)
    for batch in pbar:
        ...
        logits_masked = logits[masks]
        label_masked = labels[masks]
        loss = model.compute_loss(logits_masked.unsqueeze(0), label_masked.unsqueeze(0))
        torch.cuda.synchronize(device) if device.type == 'cuda' else None
...
def evaluate(model, dataloader, device, threshold=0.5, return_all_thresholds=False):
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="评估", mininterval=5):
            features = batch['features'].to(device)
            voronoi = batch['voronoi'].to(device)
            hb = batch['hb'].to(device)
            hp = batch['hp'].to(device)
